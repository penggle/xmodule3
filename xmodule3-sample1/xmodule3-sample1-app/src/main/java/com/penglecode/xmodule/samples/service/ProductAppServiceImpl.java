...
public class ProductAppServiceImpl implements ProductAppService {
    ...
    public Product getProductById(Long id, boolean cascade) {
        ProductBaseInfo productInfo = productInfoService.getProductById(id);
        if(productInfo != null) {
    ...
    public List<Product> getProductListByIds(List<Long> ids, boolean cascade) {
        List<ProductBaseInfo> productInfoList = productInfoService.getProductListByIds(ids);
    ...
    public List<Product> getProductListByPage(Product condition, Page page, boolean cascade) {
        List<ProductBaseInfo> productInfoList = productInfoService.getProductListByPage(condition, page);
        return prepareProductList(productInfoList, cascade);
    }

    protected List<Product> prepareProductList(List<ProductBaseInfo> productInfoList, boolean cascade) {
        if(!CollectionUtils.isEmpty(productInfoList)) {
            List<Product> productList = BeanMapper.map(productInfoList, Product::new);
            if(cascade) {
                List<Long> productIds = productInfoList.stream().map(ProductBaseInfo::getProductId).collect(Collectors.toList());
