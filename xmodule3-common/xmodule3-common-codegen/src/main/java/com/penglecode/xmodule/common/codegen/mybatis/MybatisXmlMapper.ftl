...
<mapper namespace="${mapperNamespace}">
    ...
    <sql id="SelectBaseColumnsClause">
        ...
        <#list modelQueryColumns as column>
    ...
    <sql id="UpdateDynamicColumnsClause">
        <trim suffixOverrides=",">
        <#list modelUpdateColumns as column>
            <if test="@${mapperHelperClass}@containsColumn(columns, '${column["fieldName"]}')">
    ...
    <insert id="insertModel" <#if pkGenStrategy == "IDENTITY">keyProperty="${pkFieldName}"</#if> parameterType="${domainObjectName}" statementType="PREPARED"<#if pkGenStrategy??> useGeneratedKeys="true"</#if>>
    <#if pkGenStrategy == "SEQUENCE">
        <selectKey resultType="${pkFieldType}" order="BEFORE" keyProperty="${pkFieldName}">
            SELECT ${pkGenParameter}.NEXTVAL AS ${pkFieldName} FROM dual
        </selectKey>
    </#if>
        INSERT INTO ${domainTableName}(
        <#list modelInsertColumns as column>
    ...
    <update id="updateModelById" parameterType="java.util.Map" statementType="PREPARED">
        UPDATE ${domainTableName} ${tableAliasName}
           SET <include refid="UpdateDynamicColumnsClause"/>
         WHERE <#list modelPrimaryKeyColumns as column>${tableAliasName}.${column["pkColumnName"]} = <#noparse>#{</#noparse><#if (modelPrimaryKeyColumns?size == 1)>id<#else>id.${column["pkFieldName"]}</#if>, jdbcType=${column["pkJdbcTypeName"]}<#noparse>}</#noparse><#if column_has_next> AND </#if></#list>
    </update>
    ...
    <delete id="deleteModelById" parameterType="java.util.Map" statementType="PREPARED">
        DELETE ${deleteTableAliasName} FROM ${domainTableName} ${tableAliasName}
         WHERE <#list modelPrimaryKeyColumns as column>${tableAliasName}.${column["pkColumnName"]} = <#noparse>#{</#noparse><#if (modelPrimaryKeyColumns?size == 1)>id<#else>id.${column["pkFieldName"]}</#if>, jdbcType=${column["pkJdbcTypeName"]}<#noparse>}</#noparse><#if column_has_next> AND </#if></#list>
    </delete>

    <delete id="deleteModelByIds" parameterType="java.util.Map" statementType="PREPARED">
    <#if (modelPrimaryKeyColumns?size == 1)>
        DELETE ${deleteTableAliasName} FROM ${domainTableName} ${tableAliasName}
         WHERE ${tableAliasName}.${pkColumnName} in
        <foreach collection="ids" index="index" item="id" open="(" separator="," close=")">
            <#noparse>#{</#noparse>id, jdbcType=${pkJdbcTypeName}<#noparse>}</#noparse>
        </foreach>
    <#else>
        DELETE ${deleteTableAliasName} FROM ${domainTableName} ${tableAliasName}
         WHERE <foreach collection="ids" index="index" item="id" open="" separator=" OR " close="">(<#list modelPrimaryKeyColumns as column>${tableAliasName}.${column["pkColumnName"]} = <#noparse>#{</#noparse>id.${column["pkFieldName"]}, jdbcType=${column["pkJdbcTypeName"]}<#noparse>}</#noparse><#if column_has_next> AND </#if></#list>)</foreach>
    </#if>
    </delete>
    ...
    <select id="selectModelById" parameterType="java.util.Map" resultType="${domainObjectName}" statementType="PREPARED">
        SELECT <include refid="SelectBaseColumnsClause"/>
          FROM ${domainTableName} ${tableAliasName}
         WHERE <#list modelPrimaryKeyColumns as column>${tableAliasName}.${column["pkColumnName"]} = <#noparse>#{</#noparse><#if (modelPrimaryKeyColumns?size == 1)>id<#else>id.${column["pkFieldName"]}</#if>, jdbcType=${column["pkJdbcTypeName"]}<#noparse>}</#noparse><#if column_has_next> AND </#if></#list>
    </select>
    ...
    <select id="selectAllModelCount" parameterType="java.util.Map" resultType="java.lang.Integer" statementType="PREPARED">
        SELECT COUNT(*) FROM ${domainTableName} ${tableAliasName}
    </select>
