...
import com.penglecode.xmodule.common.codegen.config.DomainIntrospectConfigProperties;
...
public class DatabaseIntrospector {
    ...
    private final DomainConfigProperties domainConfig;

    private final Map<Integer, JdbcTypeInformation> jdbcTypes;

    public DatabaseIntrospector(DomainConfigProperties domainConfig) {
    ...
    protected void overrideDefaultJdbcTypes() {
        DomainIntrospectConfigProperties introspectConfig = getDomainConfig().getDomainCommons().getIntrospectConfig();
    ...
    public Set<IntrospectedTable> introspectedTables() throws SQLException {
        ...
        Map<String,DomainObjectConfigProperties> domainObjectConfigs = getDomainConfig().getDomainObjects();
        for(Map.Entry<String,DomainObjectConfigProperties> entry : domainObjectConfigs.entrySet()) {
            ...
            introspectedTables.add(introspectedTable(domainObjectConfig));
        }
        return introspectedTables;
    }

    /**
     * 自省domainObjectConfig所指定的数据库表
     * @param domainObjectConfig
     * @return
     * @throws SQLException
     */
    public IntrospectedTable introspectedTable(DomainObjectConfigProperties domainObjectConfig) throws SQLException {
        String introspectDataSourceName = StringUtils.defaultIfBlank(domainObjectConfig.getIntrospectConfig().getIntrospectDataSource(), getDomainConfig().getDomainCommons().getIntrospectDataSource());
        ...
        try(Connection connection = introspectDataSource.getConnection()) {
            ...
            String targetTableName = domainObjectConfig.getDomainTableName();
            ...
            ImmutablePair<TableIndentity,List<IntrospectedColumn>> tableAndColumns = extractTableColumns(domainObjectConfig, databaseMetaData, tableIndentity);
            tableIndentity = tableAndColumns.getLeft();
            List<IntrospectedColumn> allColumns = tableAndColumns.getRight();
            IntrospectedTable introspectedTable = createIntrospectedTable(domainObjectConfig, databaseMetaData, tableIndentity);
            introspectedTable.setAllColumns(allColumns);
            introspectedTable.setPkColumns(extractTablePrimaryKeys(domainObjectConfig, databaseMetaData, tableIndentity, allColumns));
            for(IntrospectedColumn introspectedColumn : allColumns) {
                completeIntrospect(domainObjectConfig, introspectedColumn, introspectedTable);
            }
     ...
     * @throws SQLException
     */
    protected ImmutablePair<TableIndentity,List<IntrospectedColumn>> extractTableColumns(DomainObjectConfigProperties domainObjectConfig, DatabaseMetaData databaseMetaData, TableIndentity tableIndentity) throws SQLException {
     ...
     * @throws SQLException
     */
    protected IntrospectedTable createIntrospectedTable(DomainObjectConfigProperties domainObjectConfig, DatabaseMetaData databaseMetaData, TableIndentity tableIndentity) throws SQLException {
        ...
        introspectedTable.setDomainObjectName(domainObjectConfig.getDomainObjectName());
     ...
     * @throws SQLException
     */
    protected List<IntrospectedColumn> extractTablePrimaryKeys(DomainObjectConfigProperties domainObjectConfig, DatabaseMetaData databaseMetaData, TableIndentity tableIndentity, List<IntrospectedColumn> allColumns) throws SQLException {
    ...
    protected void completeIntrospect(DomainObjectConfigProperties domainObjectConfig, IntrospectedColumn introspectedColumn, IntrospectedTable introspectedTable) {
        ...
        DomainObjectColumnConfigProperties domainObjectColumn = domainObjectConfig.getDomainObjectColumns().get(introspectedColumn.getColumnName());
        if(domainObjectColumn != null) {
            if(domainObjectColumn.getJavaTypeOverride() != null) { //处理javaTypeOverride配置
                FullyQualifiedJavaType javaFieldType = new FullyQualifiedJavaType(domainObjectColumn.getJavaTypeOverride().getName());
    ...
    protected FullyQualifiedJavaType resolveNumberReplacement(IntrospectedColumn column, JdbcTypeInformation defaultJdbcType) {
        boolean forceNumber1AsBoolean = ObjectUtils.defaultIfNull(getDomainConfig().getDomainObjects().get(column.getIntrospectedTable().getDomainObjectName()).getIntrospectConfig().isForceNumber1AsBoolean(), getDomainConfig().getDomainCommons().getIntrospectConfig().isForceNumber1AsBoolean());
    ...
    protected FullyQualifiedJavaType resolveDateTimeReplacement(IntrospectedColumn column, JdbcTypeInformation defaultJdbcType) {
        boolean forceNumber1AsBoolean = ObjectUtils.defaultIfNull(getDomainConfig().getDomainObjects().get(column.getIntrospectedTable().getDomainObjectName()).getIntrospectConfig().isForceDateTimeAsString(), getDomainConfig().getDomainCommons().getIntrospectConfig().isForceDateTimeAsString());
    ...
    protected FullyQualifiedJavaType resolveBigDecimalReplacement(IntrospectedColumn column, JdbcTypeInformation defaultJdbcType) {
        ...
        boolean forceDecimalNumericAsDouble = ObjectUtils.defaultIfNull(getDomainConfig().getDomainObjects().get(column.getIntrospectedTable().getDomainObjectName()).getIntrospectConfig().isForceDecimalNumericAsDouble(), getDomainConfig().getDomainCommons().getIntrospectConfig().isForceDecimalNumericAsDouble());
    ...
    protected DomainConfigProperties getDomainConfig() {
        return domainConfig;
    }
}
